...
function writeBoard(board: Record<string, unknown>) {
...
ensureDir();
writeBoard(productLaunch);
writeBoard(salesPipeline);
writeBoard(salesPipelineJP);
console.log("\nDone! Created 3 demo boards with %d total cards.",
  Object.keys(productLaunch.cards).length +
  Object.keys(salesPipeline.cards).length +
  Object.keys(salesPipelineJP.cards).length
);
